...
Client L2 defaults to the v0.1 `32^3` cumulant D3Q27 path. Other solvers must be explicitly selected and are considered experimental:

```bash
AERO_LBM_CLIENT_L2_MODE=v0_1 ./gradlew runClient
AERO_LBM_CLIENT_L2_MODE=fp16_inplace ./gradlew runClient
AERO_LBM_CLIENT_L2_MODE=compact ./gradlew runClient
AERO_LBM_CLIENT_L2_ENABLED=0 ./gradlew runClient
```

`fp16_inplace` and `compact` default to 128³ bricks, and `AERO_LBM_CLIENT_L2_BRICK_SIZE` can still override the size.
...
客户端 L2 默认保持 v0.1 的 `32^3` cumulant D3Q27 路径。其它求解器需要显式选择，并视为实验性路径：
...
`fp16_inplace` 与 `compact` 默认使用 128³ brick，也可以继续用 `AERO_LBM_CLIENT_L2_BRICK_SIZE` 覆盖。`/aero_client_l2 status` 会显示实际 mode、`brickSize`、发布 stride、准备预算、step/publish 耗时和 native brick runtime 状态。`AERO_LBM_CLIENT_L2_MAX_ACTIVE_BRICKS`、`AERO_LBM_CLIENT_L2_PUBLISH_INTERVAL_TICKS`、`AERO_LBM_CLIENT_L2_PUBLISH_SAMPLE_STRIDE`、`AERO_LBM_CLIENT_L2_STATIC_BUILD_CELLS_PER_TICK`、`AERO_LBM_CLIENT_L2_COARSE_SEED_CELLS_PER_TICK` 可用于控制测试压力；默认游戏行为仍是 v0.1 的 32³。
...
| `/aero_client_l2 on/off` | 开启/关闭客户端本地 L2 解算器        |
